...
// Call API lấy thông tin khóa học theo mã
export const getInfoCourseByIdAPI = async (courseId: string) => {
    try {
        const response = await axiosClient.get(`/QuanLyKhoaHoc/LayDanhSachKhoaHoc?maKhoaHoc=${courseId}&MaNhom=GP01`);
        return response.data;
    } catch (error) {
        throw error;
    }
